function [tests ] = test_compute_features
...
tc.TestData.common_feat = {@min, @max, @mean, @std, @diff};
end

function test_output_size_def_features (tc)
exp_size = [size(tc.TestData.data), length(tc.TestData.common_feat)];
act = compute_features(tc.TestData.data);
verifySize(tc, act, exp_size);
end

function test_output_def_features (tc)
% testing only min and max features
exp_out{1} = {1.0e+03 *[8.7363    6.7705    9.2293         0], [];
              1.0e+03 *[7.3096    5.7870    6.8812         0], 1.0e+03 *[7.1644    6.2898    8.1705         0];
              1.0e+03 *[8.2679    6.0094    7.8534         0], 1.0e+03 *[6.7430    6.0849    8.5662         0];
              1.0e+04 *[1.0217    0.7916    0.9449         0], 1.0e+03 *[5.8114    6.5534    8.9444         0];
              };
exp_out{2} = {1.0e+03 *[9.3384    7.1070    9.5977    0.0011], [];
              1.0e+03 *[8.1383    6.7705    7.7296    0.0007], 1.0e+04 *[1.1402    1.0298    0.9266    0.0001];
              1.0e+04 *[1.1357    1.1628    1.8655    0.0001], 1.0e+04 *[1.2683    1.2937    1.7799    0.0002];
              1.0e+04 *[1.3791    1.4891    1.7649    0.0001], 1.0e+04 *[1.2533    1.6298    1.6097    0.0001];
    };

act = compute_features(tc.TestData.data);
verifyEqual(tc, act(:, :, 1), exp_out{1}, 'AbsTol', 1);
verifyEqual(tc, act(:, :, 2), exp_out{2}, 'AbsTol', 1);
end

function test_user_feature (tc)
...
act = compute_features(tc.TestData.data, tc.TestData.common_feat);
verifyEqual(tc, act(:, :, 1), exp_out{1}, 'AbsTol', 1);
verifyEqual(tc, act(:, :, 2), exp_out{2}, 'AbsTol', 1);
end